...
function findTonePosition(word) {
    ...
    if (cluster === 'ie' || cluster === 'iê' || cluster === 'ye' || cluster === 'yê' ||
      ...
      return vowelPositions[1]; // ALWAYS ON SECOND VOWEL
    }
    ...
    // Mặc định, dấu ở âm chính (thường là âm đầu tiên nếu không nằm trong các quy tắc đặc biệt, ví dụ: 'ai', 'ao', 'au', 'ay', 'eu', 'eo', 'oi', 'ou', 'ui', 'ua'...)
    return vowelPositions[0];
}

console.log('liê', findTonePosition('liê')); // expected 2 (index of ê)
console.log('hoa', findTonePosition('hoa')); // expected 2 (index of a)
console.log('khoe', findTonePosition('khoe')); // expected 3 (index of e)
console.log('thuy', findTonePosition('thuy')); // expected 3 (index of y)
