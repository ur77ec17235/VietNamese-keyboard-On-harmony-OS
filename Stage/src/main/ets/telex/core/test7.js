...
function applyToneToWord(word, toneIdx) {
    let result = word;
    let applied = false;

    // WAIT: in original code, does it strip existing tones?
    // Let's strip existing tones first!
    const positions = findTonePosition(result);
...
console.log('ỉêp', applyToneToWord('ỉêp', 1)); // expected iếp
